...
# # ---------------niid
# dict_users = split_data(dataset, data_train, num_of_base_learners, 'dirichlet', num_sample=len(data_train), num_classes=num_classes, alpha=1)
# train_loader_list = []
# dataiter_list = []
# for n in range(num_of_base_learners):
...
breakpoints = [20,60,300,400,500,1000]
...
for i in range(len(breakpoints)):
    # Majority voting
    acc = majority_voting(num_of_base_learners, model_list[i], data_test_loader, num_classes, device)
